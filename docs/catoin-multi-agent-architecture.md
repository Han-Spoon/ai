...
- **입력**: 사용자 GPS 좌표, (GPS가 부정확하거나 없을 때) 상호명 검색어, OCR 메뉴명 리스트(메뉴판 1장에 보통 여러 개)
- **처리**:
  ...
  2. **가게 식별**: 카카오/네이버 지도 API로 GPS 반경 내 후보 가게 리스트를 제공 → 사용자가 직접 확인/선택. 후보에 없는 신규 가게면 store_id를 새로 발급. 이 store_id는 이후 모든 하위 에이전트 호출에 필수 파라미터로 실림.
...
### ⑦ DB 업데이트 에이전트
- **역할 개요**: 이 파이프라인에서 유일하게 "AI 판단 결과를 영구 저장할지"를 결정하는 관문. 증거의 신뢰도에 따라 즉시 반영(hard evidence)과 관리자 컨펌 후 반영(soft evidence)을 명확히 구분해서 처리한다.
